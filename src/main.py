...
def aSearch(grid, window: Window):
    ...
    def trace_path(cell_details, dest):
        ...
        while not (cell_details[row][col].parent_i == row and cell_details[row][col].parent_j == col):
            path.append((row, col))
            window.grid[row][col] = NodeStatus.EXPLORED
    ...
    def a_star_search(grid, src, dest):
        ...
        while len(open_list) > 0:
            ...
            window.grid[i][j] = NodeStatus.EXPLORING
            window.drawGrid(window.grid)
    ...
    def main():
        ...
        for i in range(40):
            grid.append([])
            for j in range(40): 
                grid[i].append(int(not window.grid[i][j]))
        # Define the source and destination
        grid[window.start[0]][ window.start[1]]=1
        grid[window.end[0]][ window.end[1]]=1
        src = [window.start[0], window.start[1]]
        dest = [window.end[0], window.end[1]]
